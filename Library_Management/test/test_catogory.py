...
@pytest.mark.asyncio
async def test_delete_catogory_success(
    test_app, admin_test_user_token, create_category
):
    transport = ASGITransport(app=test_app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        headers = {"Authorization": f"Bearer {admin_test_user_token}"}
...
async def test_update_catogory_success(
    test_app, admin_test_user_token, create_category
):
    transport = ASGITransport(app=test_app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        response_2 = await client.put(
            f"/category/update/{id}", headers=headers, json=payload
        )
