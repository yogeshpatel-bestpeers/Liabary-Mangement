...
from Library_Management.database import get_db,IssuedBook,Book
...
@issuedBook.post('/book/issue/{student_id}/{book_id}', status_code=status.HTTP_201_CREATED, tags=['Issued Books Management'])
def issued_book(student_id: str, book_id: str, db: Session = Depends(get_db)):
    ...
    issued_book = db.query(IssuedBook).filter(IssuedBook.book_id == book_id, IssuedBook.user_id == student_id).first()
...
def return_book(student_id: str, book_id: str, db: Session = Depends(get_db)):
    ...
    issued_book = db.query(IssuedBook).filter(
    ...
    if not issued_book:
        raise HTTPException(detail='Issued Book Not Found or Already Returned', status_code=status.HTTP_404_NOT_FOUND)
