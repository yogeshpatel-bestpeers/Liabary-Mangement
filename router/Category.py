...
def auther_get(db : Session =Depends(get_db)):
  category = db.query(database.Category).options(joinedload(database.Category.books)).all()
...
def category_update(id : str,model: models.Category_Created,db : Session =Depends(get_db)):
